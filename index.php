  ...
  <div class="grid airport-view" style="display:none;">
    <section class="board">
      <div class="boardHeader">
        <h2 class="js-depTitle">Departures</h2>
        <div class="mini">
          <span class="hint js-sortHint">Sort:</span>
          <select class="js-depSort"></select>
          <button class="js-depDir" title="Reverse sort">▲</button>
        </div>
      </div>
      <table class="js-depTable">
        <thead><tr class="js-depHeadRow"></tr></thead>
        <tbody class="js-depBody"></tbody>
      </table>
    </section>

    <section class="board">
      <div class="boardHeader">
        <h2 class="js-arrTitle">Arrivals</h2>
        <div class="mini">
          <span class="hint js-sortHint">Sort:</span>
          <select class="js-arrSort"></select>
          <button class="js-arrDir" title="Reverse sort">▲</button>
        </div>
      </div>
      <table class="js-arrTable">
        <thead><tr class="js-arrHeadRow"></tr></thead>
        <tbody class="js-arrBody"></tbody>
      </table>
    </section>
  </div>
</template>
...
(() => {
  ...
  const I18N = {
    de: {
      ...
      th_type_route: 'Typ <span class="opt-route muted">/ Route</span>',
      th_alt_gs: "Alt/GS",
      th_status: "Status",
      th_xp: "XP",
      ...
      cfg_hide_local: "Lokalflüge ausblenden",
    ...
    en: {
      ...
      th_from: "From",
      th_callsign: "Callsign",
      th_type_route: 'Type <span class="opt-route muted">/ Route</span>',
      th_alt_gs: "Alt/GS",
      th_status: "Status",
      th_xp: "XP",
      ...
      cfg_hide_pred: "Hide predictive runways",
      cfg_hide_local: "Hide local flights",
      cfg_hide_vfr: "Hide VFR traffic",
...
const GATE_JITTER_GRACE_MS = 20 * 1000;    // kurzzeitig minimal bewegt -> Gate behalten
  ...
  // Database / Cache Keys for new Features
...
// --- NEU: Eindeutige Session-ID generieren und im Session-Kontext speichern ---
 ...
 async function loadMemberCache() {
  ...
  let rexRules = {}; // Regelwerk für Runway-Prediction
  let currentVatsimData = null; // Speichert den letzten Feed für Config-Reloads
  // Global store for active runways per airport (for ETD calc)
  let globalActiveRunways = {};
  ...
  function createSortOptionsHtml(currentVal, boardType){
     const opts = [
       ...
       ["time", boardType==='dep' ? t("sort_time_std") : t("sort_time_eta")],
       ["callsign", t("sort_callsign")],
       ["dest", boardType==='dep' ? t("sort_dest") : t("sort_orig")],
       ["xp", t("sort_xp")]
     ];
     return opts.map(([v,txt]) => `<option value="${v}" ${v===currentVal?'selected':''}>${escapeHtml(txt)}</option>`).join("");
  }

  /* +     Legacy global elements removed/replaced by dynamic scoping.
     The sort rebuilding logic is moved to setupAirportView.
  */
...
// --- WAKE LOCK LOGIC ---
  ...
  function applyLanguage(){
    ...
    for(const [icao, boardSet] of airportBoards){
        // Update headers/labels in DOM
        buildHeaderFor("dep", boardSet.ui.depHeadRow);
        buildHeaderFor("arr", boardSet.ui.arrHeadRow);
        boardSet.ui.depSortHint.textContent = t("sort_hint");
        boardSet.ui.arrSortHint.textContent = t("sort_hint");
        boardSet.ui.depSort.innerHTML = createSortOptionsHtml(settings.depSort.by, "dep");
        boardSet.ui.arrSort.innerHTML = createSortOptionsHtml(settings.arrSort.by, "arr");
        boardSet.ui.depDir.title = t("dep_dir_title");
        boardSet.ui.arrDir.title = t("arr_dir_title");

        // Trigger re-render to update column headers and XP tooltips
        boardSet.depBoard.render(boardSet.lastDeps || []);
        boardSet.arrBoard.render(boardSet.lastArrs || []);
    }
  ...
  // Eigener Fetcher für METAR (Bypass global limiter)
  async function fetchMetarDirect(icao) {
    icao = normalizeCode(icao);
    // Cache-Buster um aggressive Browser-Caches zu umgehen
    const url = METAR_API_URL.replace("{icao}", icao) + "&_" + Date.now();
    // Direkt via Proxy ohne Queue
    ...
    try {
      ...
      console.warn("METAR fetch err:", e);
      return null;
    }
  }
...
async function updateAirportRunwayInfo(atisList = []) {
    // 1. Laden der Caches
    const [metarCache, rwyCache] = await Promise.all([
        loadDataFromDbOrLocal(METAR_CACHE_KEY),
        loadDataFromDbOrLocal(ACT_RUNWAY_CACHE_KEY)
    ]);


    const now = Date.now();
    let metarDirty = false;
    let rwyDirty = false;


    // Wir iterieren über ALLE konfigurierten Airports, nicht nur den aktiven
    const airports = settings.airports;

    for (const ap of airports) {
        const icao = normalizeCode(ap);
        if (!icao) continue;

        // FLICKER FIX & STALE-WHILE-REVALIDATE
        // 1. Immer anzeigen was wir haben (auch wenn alt), damit UI stabil bleibt
        ...
        // ALLE ATIS Stationen suchen (A_ATIS, D_ATIS, oder normale ATIS)
        const stations = atisList.filter(a => a.callsign.startsWith(icao + "_") && a.callsign.endsWith("_ATIS"));
        const atisOnline = stations.length > 0;

        let rwyInfo = null;
        let atisCode = null;
        let needsUpdate = true;

        // Check Age: Wenn Daten frisch (< 20 Min) und ATIS vorhanden (oder PRED valid), überspringen wir Fetch
        // ABER: Wenn ATIS online ist, wollen wir sofort updaten (falls sich Buchstaben/Runways geändert haben).
        if (cachedRwy) {
            const age = now - (cachedRwy.ts || 0);
            // Wenn ATIS da ist -> Immer parsen (Parsing ist billig, Daten sind im Feed).
            // Wenn KEINE ATIS da ist -> Cache für 20min nutzen (spart METAR API Calls).
            if (!atisOnline && age < 20 * 60 * 1000) {
                needsUpdate = false;
            }
        }
        ...
        // METAR Update nötig?
        // Nur fetchen, wenn wir wirklich müssen (kein Cache oder alt) UND wir keine ATIS haben (für Prediction Fallback)
        // ODER wenn wir ATIS haben aber trotzdem Winddaten für History wollen (optional, hier sparen wir API Calls).
        let mData = metarCache[icao];

        // Wir holen METAR, wenn Cache alt ist. Das ist unabhängig von ATIS, da wir Winddaten für Predictions anderer Features brauchen könnten.
        // Da METAR fetch async ist, blockiert es hier nicht.
        if (!mData || (now - mData.ts) > 20 * 60 * 1000) {
            // Wir fetchen asynchron, blockieren aber hier nicht ewig die UI
            // In einer idealen Welt würden wir das Promises array sammeln und waiten,
            // aber wir wollen schnelle Response. Wir triggern den Fetch, beim nächsten Interval ist er da.
            // ODER wir warten kurz, da wir async sind.
            const freshMetar = await fetchMetarDirect(icao);
            if (freshMetar) {
                console.log(`RWY DEBUG ${icao}: Fresh METAR fetched`, freshMetar.rawOb);
                mData = {
                    metar: freshMetar.rawOb || "",
                    wind: { dir: freshMetar.wdir, spd: freshMetar.wspd },
                    qnh: freshMetar.altim,
                    ts: now
                };
                metarCache[icao] = mData;
                metarDirty = true;
            }
        }

        if (stations.length > 0) {
            // Merge Logic für Split-ATIS (z.B. KDFW Arr/Dep)
            let combinedArr = [];
            let combinedDep = [];
            let codes = new Set();

            for(const st of stations){
                const parsed = parseActiveRunways(st);
                if(parsed.arrivals) combinedArr.push(...parsed.arrivals);
                if(parsed.departures) combinedDep.push(...parsed.departures);
                if(st.atis_code) codes.add(st.atis_code);
            }

            rwyInfo = {
                icao: icao,
                arrivals: [...new Set(combinedArr)].sort(),
                departures: [...new Set(combinedDep)].sort(),
                atisCode: Array.from(codes).sort().join("/")
            };

            // Codes joinen (z.B. "V" und "X" -> "V / X")
            atisCode = rwyInfo.atisCode;

            // FALLBACK: Wenn ATIS da ist, aber Parsing keine Runways lieferte (0/0)
            // dann versuchen wir Prediction/Fallback, markieren es aber.
            if ((!rwyInfo.arrivals.length && !rwyInfo.departures.length) && !settings.hidePredictiveRunways && mData) {
            console.log(`RWY DEBUG ${icao}: ATIS empty parse, trying prediction...`);
                        // Nur bei Single-Station ATIS Fallback probieren, bei Split-ATIS ist 0/0 unwahrscheinlich wenn Text da ist
                 await ensureRunwayIndexForIcao(icao).catch(()=>{});
                 const pred = predictRunways(icao, mData, rwyCache[icao]?.history);
                 if (pred) {
                     rwyInfo.arrivals = pred.arrivals;
                     rwyInfo.departures = pred.departures;
                     rwyInfo.source = "PRED (ATIS Parse Fail)";
                 }
            } else {
                rwyInfo.source = "ATIS";
            }
            ...
            rwyCache[icao].current = { ...rwyInfo, ts: now, atisCode };

            // Nur in History schreiben, wenn wir wirklich etwas gefunden haben (kein leeres ATIS)
            if (rwyInfo.arrivals.length > 0 || rwyInfo.departures.length > 0) {
                rwyCache[icao].history.push({ ts: now, rwy: rwyInfo, wind: mData?.wind });
                // History trimmen (6h)
                rwyCache[icao].history = rwyCache[icao].history.filter(h => (now - h.ts) < 6 * 3600 * 1000);
            }
            rwyDirty = true;
    ...
    // Speichern
    if (metarDirty) saveDataToDbAndCleanLocal(METAR_CACHE_KEY, metarCache);
    if (rwyDirty) saveDataToDbAndCleanLocal(ACT_RUNWAY_CACHE_KEY, rwyCache);

}

function renderLoadingRunways(icao) {
    if (icao !== settings.activeTab) return;
    const container = document.getElementById("globalRunways");
    if(container) {
        container.className = "runwayGlobal";
        container.innerHTML = `<span style="opacity:0.7; font-size:12px; font-family:var(--font-mono);">Loading runways...</span>`;
    }
}
...
async function fetchOverpassTaxiwaysForAirport(icao){
  ...
  async function loadRexRules(){
    try {
        const res = await fetch(REX_RULES_URL);
        if(res.ok) rexRules = await res.json();
        console.log("REX Rules loaded:", Object.keys(rexRules).length, "airports");
    } catch(e) { console.warn("REX Rules load failed", e); }
  }
...
function saveRunwayCache(obj){
  ...
  // NEW: Helper to stitch displaced thresholds (unnamed small chunks) onto main runways
  function stitchRunwayGeometry(elements) {
    const ways = elements.filter(e => e.type === "way" && Array.isArray(e.geometry) && e.geometry.length >= 2);
    const named = [];
    const orphans = [];

    // 1. Separate Main Runways (Ref/Name) from Orphans
    for(const w of ways) {
        const tags = w.tags || {};
        if(tags.ref || tags.name) named.push(w);
        else orphans.push(w);
    }

    // 2. Try to stitch orphans to named runways
    // We assume the orphan extends the runway at one end.
    // Criteria: Distance < 30m AND Angle alignment < 10 degrees.
    for(const orphan of orphans) {
        const oStart = orphan.geometry[0];
        const oEnd   = orphan.geometry[orphan.geometry.length-1];
        const oBrg   = bearingDeg(oStart.lat, oStart.lon, oEnd.lat, oEnd.lon);

        let merged = false;
        for(const main of named) {
            const mStart = main.geometry[0];
            const mEnd   = main.geometry[main.geometry.length-1];
            const mBrg   = bearingDeg(mStart.lat, mStart.lon, mEnd.lat, mEnd.lon);

            // Check alignment (must be parallel-ish)
            const angDiff = angleDiffDeg(oBrg, mBrg);
            if(angDiff > 10 && angDiff < 170) continue; // Not aligned

            // Check Proximity: Orphan End <-> Main Start
            if(haversineMeters(oEnd.lat, oEnd.lon, mStart.lat, mStart.lon) < 30) {
                // Prepend orphan points to main
                main.geometry = [...orphan.geometry, ...main.geometry];
                merged = true;
            }
            // Check Proximity: Orphan Start <-> Main End
            else if(haversineMeters(oStart.lat, oStart.lon, mEnd.lat, mEnd.lon) < 30) {
                // Append orphan points to main
                main.geometry = [...main.geometry, ...orphan.geometry];
                merged = true;
            }
            // Also check inverted matching (if orphan is drawn reverse) -> simplified:
            // Since usually segments are drawn roughly consistent or short, the endpoint check covers most.

            if(merged) break; // Consumed this orphan
        }
    }
    return named; // Return only the now-extended main runways
  }

  function buildRunwayEndpointsFromElements(elements){
    const endpoints = [];

    // 1. Stitch fragments before processing
    const consolidated = stitchRunwayGeometry(elements || []);
 ...
 async function fetchOverpassRunwaysForAirport(icao){
  ...
  function projectToRunwayAxisMeters(pLat, pLon, aLat, aLon, bLat, bLon){
    ...
    // FIX: Do NOT clamp t to 0..1 here. We need the projection onto the infinite line
    // (extended centerline) for correct approach detection.
    // t < 0 = Approach/Before Threshold, t > 1 = Past Stop End.
...
function resolveArrivalRunwayForFlight(f, runwayIndex, activeRunways, distNm, elevation=0){
  ...
  function pickBestRunwayEndpoint(cid, lat, lon, runwayIdx, activeRunways){
    ...
    for(const ep of runwayIdx.endpoints){
      ...
      // Active Runway Weighting
      // If ATIS/Prediction says this is active, boost it significantly.
      let weightBonus = 0;
      if(activeRunways && activeRunways.departures && activeRunways.departures.includes(ep.designator)) {
          const isPred = (activeRunways.source || "").includes("PRED");
          // Massive bonus for ATIS confirmed (8km), large for Pred (4km)
          weightBonus = isPred ? 4000 : 8000;
      }
      const weightedMetric = metricNow - weightBonus;

      if(!isFinite(dNow) || dNow > RUNWAY_TARGET_MAX_CONSIDER_M) continue;
      if(!isFinite(metricNow)) continue;

      if(!bestAny || weightedMetric < bestAny.metric){
        bestAny = { ep, dNow, metric: weightedMetric, axisLatM: mNow.axisLat, approaching:false, gainM:0 };
      }
      ...
      if(prevOk){
        ...
        if(isFinite(gainM) && gainM >= RUNWAY_APPROACH_MIN_GAIN_M){
          if(!bestApproach || weightedMetric < bestApproach.metric){
            bestApproach = { ep, dNow, metric: weightedMetric, axisLatM: mNow.axisLat, approaching:true, gainM };
          }
        }
      }
    }
...
function calculateFlightLevelString(altitude, qnhHg) {
  ...
  function resolveRunwayTargetForCid(cid, lat, lon, runwayIdx, activeRunways){
    ...
    const best = pickBestRunwayEndpoint(cid, lat, lon, runwayIdx, activeRunways);
    if(!best) return null;
  ...
  function getRunwayTargetForFlight(f, runwayIdx, activeRunways){
    if(!f || !runwayIdx) return null;
    const p = f.pilot || null;
    const lat = Number(p?.latitude), lon = Number(p?.longitude);
    if(!isFinite(lat) || !isFinite(lon)) return null;

    const r = resolveRunwayTargetForCid(f.cid, lat, lon, runwayIdx, activeRunways);
    if(!r?.ep) return null;
  ...
  function getDepartureQueueDelayForFlight(f, runwayIdx, queueKey){
    ...
    for(const g of (lastDeps || [])){
      ...
      const tgt = getRunwayTargetForFlight(g, runwayIdx, globalActiveRunways[currentAirportIcao]);
...
function fmtRegDate(val) {
  ...
  if(boardType === "dep" && recentlyLandedHere && arr === focusIcao && dep !== focusIcao) return { phase:"UNK", rank:99, text:"—", cls:"hidden", finished:true };
    ...
    // --- FIX: ROBUSTE SINKFLUG-ERKENNUNG ---
    // 1. Cruise Altitude aus dem Flugplan lesen
    let cruiseAlt = 30000;
    if (fp && fp.altitude) {
    ...
    // 2. Geometrischer Sinkflug: Deutlich unter Cruise Alt, nah am Ziel (<200NM), näher am Ziel als am Start, und steigt nicht stark
    ...
    // 3. Aktiv wenn: Physisch sinkend, ODER geometrisch im Sinkflug, ODER Hysterese (war im Sinkflug und steigt nicht)
...
// --- GO-AROUND OPTIMIERUNG ---
    ...
    const isClimbingStrong = (vr != null && vr > 800); // min 800 fpm
...
try{
 ...
 if(boardType === "arr"){
      ...
      if(focus && arr === focus){
        ...
        updateArrActualTimesForPilot(p, {
            isLandedAtFocus: landedHere,
            isInBlockAtFocus: inBlockHere,
            reset: shouldReset
        });
      }
    }
...
function computeDynamicEtaTs(f, destAp, phase, targetCoords=null, rwyDes=null, rIdx=null){
  ...
  function applyEtaForFlight(f, focusAp, boardType){
    ...
    if(f.status?.finished){
      ...
      // Use detected runway threshold for precise ETA if available
...
// Progress Blending (Live vs Plan) with Sanity Check
      ...
      if(!isFinite(cand)) cand = f.etaPlannedTs ?? null;
  ...
  function applyDestEtaForDepFlight(f){
    ...
    if(f.status?.finished){
      ...
      // Solution: Blend 'Live Physics ETA' with 'Plan-Relative ETA' based on flight progress.
      if(isFinite(cand) && f.fp?.enroute_time){
         const distFlown = f.dFromFocusNm || 0;
         const distRem = f.dToPlanNm || 0;
         const totalDist = distFlown + distRem;

         // Calculate Progress (0.0 to 1.0)
         const progress = (totalDist > 5) ? (distFlown / totalDist) : 0;

         // Calculate "Plan-Relative ETA": Now + (TotalPlanTime * RemainingPercentage)
         // This is robust against delays because it calculates relative to "Now" and distance left.
         const durMin = parseDurationHHMM(f.fp.enroute_time);
         if(durMin){
             const remainingPlanMs = (durMin * 60000) * (1.0 - progress);
             const planEta = now + remainingPlanMs;

             // Weighting: Trust Plan heavily at start (progress < 20%), then fade to Live.
             // Cubic curve for smooth transition.
             let weight = (progress < 0.20) ? Math.pow(1.0 - (progress / 0.20), 3) : 0;
      ...
      // Fallback if physics failed completely
      if(!isFinite(cand)){
        cand = f.etaPlannedTs ?? null;
      }
      if(isFinite(cand) && cand < (now - 2*60*1000)) cand = now + 2*60*1000;
...
function depDisplayTimeTs(f){
    ...
    // FIX: Auch bei LINEUP wollen wir STD/ETD sehen, nicht die Ankunftszeit
    if(phase === "GATE" || phase === "PREFILE" || phase === "LINEUP") {
...
function sortFlights(list, {by, dir}, boardType){
  ...
  class Board {
    constructor(tbody, boardType, titleEl){
      this.tbody = tbody;
    ...
    updateRow(tr, f){
      ...
      // Helper für hh:mm Zeitformatierung
      const formatRemTime = (mins) => {
...
if(c.altgs){
 ...
 +   * Airport Pilot XP Index (Logic & Tooltip)
  ...
  function updateTabsXpMetadata(){
     // iterate all active airport boards
     for(const [icao, boardSet] of airportBoards){
        const depIdx = computeBoardXpIndex(boardSet.lastDeps || []);
        const arrIdx = computeBoardXpIndex(boardSet.lastArrs || []);
        boardSet.xpStats = { dep: depIdx, arr: arrIdx };
     }
        }
  ...
  // NEW: Airport Boards Container
  // Map<ICAO, { container, depBoard, arrBoard, lastDeps:[], lastArrs:[], ui:{...} }>
  const airportBoards = new Map();

  // Global refs kept for legacy helper access if needed, but primary source is now airportBoards
  ...
  function setupAirportView(icao){
     ...
     const container = clone.querySelector('.airport-view');
     container.dataset.icao = icao;

     // Scope UI lookups to this clone
     const q = (sel) => container.querySelector(sel);
     const ui = {
        depTitle: q('.js-depTitle'),
        arrTitle: q('.js-arrTitle'),
        depSort: q('.js-depSort'),
        arrSort: q('.js-arrSort'),
        depDir: q('.js-depDir'),
        arrDir: q('.js-arrDir'),
        depHeadRow: q('.js-depHeadRow'),
        arrHeadRow: q('.js-arrHeadRow'),
        depBody: q('.js-depBody'),
        arrBody: q('.js-arrBody'),
        depTable: q('.js-depTable'),
        arrTable: q('.js-arrTable'),
        depSortHint: q('.boardHeader .mini .hint'), // assuming first one is dep
        arrSortHint: q('.board:nth-child(2) .boardHeader .mini .hint')
     };

     // Setup Sort Options
     ui.depSort.innerHTML = createSortOptionsHtml(settings.depSort.by, 'dep');
     ui.arrSort.innerHTML = createSortOptionsHtml(settings.arrSort.by, 'arr');
     ui.depDir.textContent = settings.depSort.dir === 1 ? "▲" : "▼";
     ui.arrDir.textContent = settings.arrSort.dir === 1 ? "▲" : "▼";
     ui.depDir.title = t("dep_dir_title");
     ui.arrDir.title = t("arr_dir_title");

     // Setup Headers
     buildHeaderFor("dep", ui.depHeadRow);
     buildHeaderFor("arr", ui.arrHeadRow);

     // Instantiate Boards
     const depBoard = new Board(ui.depBody, "dep", ui.depTitle);
     const arrBoard = new Board(ui.arrBody, "arr", ui.arrTitle);

     // Attach Events (Scoped to this view)
     // When sorting changes, we update global settings AND re-sort THIS board immediately.
     // Note: This changes settings for ALL boards, as FIDS usually have uniform settings.
     const triggerSort = (which) => {
        saveSettings();
        // Re-sort and render this specific board immediately
        const entry = airportBoards.get(icao);
        if(entry){
            if(which === 'dep') {
                entry.lastDeps = sortFlights(entry.lastDeps, settings.depSort, "dep");
                entry.depBoard.render(entry.lastDeps);
            } else {
                entry.lastArrs = sortFlights(entry.lastArrs, settings.arrSort, "arr");
                entry.arrBoard.render(entry.lastArrs);
            }
        }
        // Also trigger generic resort for others? Optional.
        // Since we refresh every 15s, other tabs will update eventually, or we could loop all.
        // For now, updating current is enough feedback.
     };

     ui.depSort.addEventListener("change", () => { settings.depSort.by = ui.depSort.value; triggerSort('dep'); });
     ui.depDir.addEventListener("click", () => { settings.depSort.dir *= -1; ui.depDir.textContent = settings.depSort.dir === 1 ? "▲" : "▼"; triggerSort('dep'); });

     ui.arrSort.addEventListener("change", () => { settings.arrSort.by = ui.arrSort.value; triggerSort('arr'); });
     ui.arrDir.addEventListener("click", () => { settings.arrSort.dir *= -1; ui.arrDir.textContent = settings.arrSort.dir === 1 ? "▲" : "▼"; triggerSort('arr'); });

     attachTooltipDelegation(ui.depBody);
     attachTooltipDelegation(ui.arrBody);

     boardsContainer.appendChild(container);

     const entry = { container, depBoard, arrBoard, lastDeps:[], lastArrs:[], ui, xpStats:null };
     airportBoards.set(icao, entry);
     return entry;
  }
  ...
  async function initTabs(){
     ...
     for(const icao of list) setupAirportView(icao);

     // 2. Remove unused views
     for(const [icao, entry] of airportBoards){
         if(!list.includes(icao)){
             entry.container.remove();
             airportBoards.delete(icao);
         }
     }
     // Sync active tab
     ...
     for(let i=0; i<3; i++){
        ...
        if(code){
           ...
           btn.addEventListener("mouseover", (e) => {
              const entry = airportBoards.get(code);
              if(!entry || !entry.xpStats) return;
              const xp = entry.xpStats;
...
<div style="display:grid; white-space:normal; grid-template-columns: auto auto auto auto; gap:2px 8px; align-items:center; font-family:var(--font-mono); font-size:11px;">
  ...
  function switchTab(icao){
    ...
    for(const [code, entry] of airportBoards){
        entry.container.style.display = (code === icao) ? "grid" : "none";
    }

    // Legacy global sync for sorting/XP logic usage
    const activeEntry = airportBoards.get(icao);
    if(activeEntry){
        lastDeps = activeEntry.lastDeps;
        lastArrs = activeEntry.lastArrs;
        currentAirportIcao = icao;
    }

     // Show spinner immediately if we don't have runway data yet
     if (!globalActiveRunways[icao]) {
         renderLoadingRunways(icao);
     }
     // Trigger background update (debounce handled inside)
     if (currentVatsimData) updateAirportRunwayInfo(currentVatsimData.atis || []);
  }
  ...
  function updateTabUi(){
    ...
    // Init Visibility
    for(const [code, entry] of airportBoards){
        entry.container.style.display = (code === settings.activeTab) ? "grid" : "none";
    }
  }
...
async function fetchFeed(){
  ...
  function scheduleResort(which){
    ...
    resortTimers[which] = setTimeout(async () => {
      ...
      // Iterate all active boards and re-sort/render if XP sorting is active
      for(const boardSet of airportBoards.values()){
          if(which === "dep"){
            boardSet.lastDeps = sortFlights(boardSet.lastDeps, settings.depSort, "dep");
            boardSet.depBoard.render(boardSet.lastDeps);
          }else{
            boardSet.lastArrs = sortFlights(boardSet.lastArrs, settings.arrSort, "arr");
            boardSet.arrBoard.render(boardSet.lastArrs);
          }
      }
      scheduleAirportXpIndexUpdate();
  ...
  async function refresh(){
    try{
      ...
      for(const airportIcao of settings.airports){
          const boardSet = airportBoards.get(airportIcao);
          if(!boardSet) continue; // should not happen if initTabs called
          ...
          for(const p of pilots){
            ...
            if(settings.hideLocalFlights && dep === arr) continue;

            // Filter: VFR
            if(settings.hideVfrFlights && fp?.flight_rules === "V"){
              continue;
            }
          ...
          if(settings.showPrefiles){
             for(const pf of prefiles){
               ...
               // Filter: Local Flights (Prefile)
               if(settings.hideLocalFlights && dep === arr) continue;
               // Filter: VFR (Prefile)
               if(settings.hideVfrFlights && fp?.flight_rules === "V") continue;
          ...
          boardSet.lastDeps = sortFlights(deps, settings.depSort, "dep");
          boardSet.lastArrs = sortFlights(arrs, settings.arrSort, "arr");

          // Render this board immediately (in background or foreground)
          boardSet.depBoard.render(boardSet.lastDeps);
          boardSet.arrBoard.render(boardSet.lastArrs);
      ...
      // Runways / ATIS Update
      const atisList = (data && Array.isArray(data.atis)) ? data.atis : [];
...
// Legacy Listeners removed; they are now scoped in setupAirportView
